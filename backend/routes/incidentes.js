...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const result = await pool.query(
            'DELETE FROM incidentes WHERE id = $1 AND municipio_id = $2 RETURNING id',
            [id, req.municipioId]
        );
        ...
        res.json({ success: true, mensaje: 'Incidente eliminado' });
        
    } catch (error) {
        console.error('❌ Error en DELETE /incidentes:', error);
